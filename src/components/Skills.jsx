...
const getSkillShape = (level) => {
  if (level >= 85) return { type: 'hexagon', gradient: 'from-orange-500 via-red-500 to-red-600', glow: 'rgba(255, 87, 34, 0.5)' };
  if (level >= 70) return { type: 'octagon', gradient: 'from-purple-500 via-pink-500 to-pink-600', glow: 'rgba(156, 39, 176, 0.5)' };
  if (level >= 60) return { type: 'diamond', gradient: 'from-blue-500 via-cyan-500 to-cyan-600', glow: 'rgba(33, 150, 243, 0.5)' };
  return { type: 'circle', gradient: 'from-green-500 via-teal-500 to-teal-600', glow: 'rgba(76, 175, 80, 0.5)' };
};

const getBorderRadius = (type) => {
  if (type === 'hexagon') return '1rem';
  if (type === 'octagon') return '1.5rem';
  if (type === 'diamond') return '0.5rem';
  return '50%';
};

// Pre-compute particle offsets so they don't change every hover
...
const Skills = () => {
  ...
  const categories = useMemo(() => ['All', ...new Set(skills.map((skill) => skill.category))], []);
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-[#DC143C] text-sm font-bold tracking-wider">TECHNOLOGIES</span>
          </motion.div>
          <motion.h2 className="text-5xl sm:text-7xl font-bold mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
            <span className="text-white">Tech </span>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#DC143C] to-[#FF1744]">Skills</span>
          </motion.h2>
          <motion.div
            initial={{ scaleX: 0 }}
          ...
          />
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">A diverse set of technologies I use to build modern applications</p>
        </motion.div>
        ...
        {/* Removed `layout` from grid — it causes expensive re-layout on every filter change */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 mb-16">
          <AnimatePresence mode="popLayout">
            {filteredSkills.map((skill, index) => (
              <SkillBubble key={skill.name} skill={skill} index={index} inView={inView} />
            ))}
          </AnimatePresence>
        </div>
...
const SkillBubble = React.memo(({ skill, index, inView }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.35, delay: Math.min(index * 0.025, 0.5), type: 'spring', stiffness: 220 }}
    ...
    >
      <motion.div
        className={`relative p-6 bg-gradient-to-br ${shape.gradient} shadow-xl aspect-square flex items-center justify-center text-center overflow-hidden`}
        style={{
      ...
      >
        <div className="text-white font-bold text-sm sm:text-base leading-tight relative z-10">
          {skill.name}
        </div>
        {/* Static inner glow — no animation when not hovered */}
        <div
          className="absolute inset-0 opacity-25 transition-opacity duration-300"
          style={{ background: `radial-gradient(circle, ${shape.glow}, transparent)` }}
        />
        {/* Hover pulse ring — CSS-only, not framer-motion */}
        {isHovered && (
          <motion.div
      ...
      {/* Pre-computed orbit particles — no Math.random in render */}
      {isHovered && PARTICLE_OFFSETS.map((offset, i) => (
        <motion.div
          key={i}
          className="absolute w-2 h-2 rounded-full"
          style={{
            background: `linear-gradient(135deg, ${shape.glow}, white)`,
            top: '50%',
            left: '50%',
          }}
          initial={{ scale: 0, x: 0, y: 0, opacity: 1 }}
          animate={{ scale: [0, 1, 0], x: offset.x, y: offset.y, opacity: [1, 0.5, 0] }}
