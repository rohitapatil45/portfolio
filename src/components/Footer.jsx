  ...
  return (
    <footer className="bg-[#0a0a0a] border-t border-gray-800 relative overflow-hidden">
      ...
      <div className="absolute inset-0">
        ...
        {/* Reduced floating particles - 5 instead of 10 */}
        {[...Array(5)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-1 h-1 bg-[#DC143C] rounded-full"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          ...
          >
            <motion.div
              whileHover={{ scale: 1.05 }}
              className="text-3xl font-bold text-white inline-block cursor-pointer group relative"
              style={{ fontFamily: "'Space Grotesk', sans-serif" }}
            >
              <span className="text-[#DC143C]">{'<'}</span>
              {personalInfo.name.split(' ')[0]}
              ...
              <motion.div
                className="absolute -inset-2 bg-[#DC143C]/15 rounded-lg blur-lg opacity-0 group-hover:opacity-100 transition-opacity -z-10"
              />
            </motion.div>
            <p className="text-gray-400 leading-relaxed">
              Full Stack Developer passionate about building scalable web
        ...
        >
          ...
          >
            ...
            {/* Simplified glow effect */}
            <motion.div
              className="absolute inset-0 bg-[#DC143C] rounded-full blur-xl opacity-40 group-hover:opacity-60 transition-opacity"
              animate={{
                scale: [1, 1.15, 1],
              }}
              transition={{ duration: 2.5, repeat: Infinity }}
...
const SocialButton = React.memo(({ href, icon: Icon, label }) => {
  return (
    <motion.a
      href={href}
      target="_blank"
    ...
    >
      ...
      {/* Background glow */}
      <motion.div
        className="absolute inset-0 bg-[#DC143C]/15 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity"
      />
